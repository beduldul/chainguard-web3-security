...
export function generateAiExplanation(
  ...
  if (locale === 'id') {
    if (riskLevel === 'CRITICAL' || riskLevel === 'HIGH') {
      if (decodedTx.isUnlimitedApproval) {
        return `Transaksi ini meminta izin untuk memindahkan aset ${decodedTx.contractName || 'Token'} tanpa batas (UNLIMITED approval) ke kontrak unverified (${decodedTx.spenderAddress?.slice(0, 6)}...${decodedTx.spenderAddress?.slice(-4)}). Jika kontrak ini diretas atau merupakan scam, peretas dapat menguras seluruh saldo wallet Anda sebesar estimasi $4,820 USDC kapan saja tanpa konfirmasi tambahan. Ditolak sangat disarankan!`;
      }
      return `Transaksi berisiko tinggi (${score}/100). Anda berinteraksi dengan smart contract unverified yang baru dibuat ${contractReport.ageDays} hari lalu. Kontrak ini memiliki hak admin penuh yang dapat mengubah logika kontrak secara sepihak.`;
    }
  ...
  if (riskLevel === 'CRITICAL' || riskLevel === 'HIGH') {
    if (decodedTx.isUnlimitedApproval) {
      return `High Security Risk (${score}/100): This transaction requests UNLIMITED spending permission for your ${decodedTx.contractName || 'tokens'} to spender contract (${decodedTx.spenderAddress?.slice(0, 6)}...${decodedTx.spenderAddress?.slice(-4)}). You do NOT need to grant unlimited approval to complete this transaction. If this contract is malicious or compromised, your entire wallet balance of up to $4,820 USDC can be drained remotely without any further popup!`;
    }
    return `High Risk Transaction Detected (${score}/100). Target contract is unverified on-chain and was deployed only ${contractReport.ageDays} days ago. An active owner/admin key was detected, allowing implementation upgrades or funds withdrawal.`;
  }
